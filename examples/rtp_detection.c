...
static bool compress_with_callback(struct rohc_comp *const compressor,
                                   const struct rohc_buf packet, int packet_count)
...
//! [define random callback 1]
	...
	for(j=0;j<100;j++){
		...
		if(j==0)
			printf("***RTP: uncompressed packet length:\t%d", ip_packet.len);

		/* compress the RTP packet with a user-defined callback to detect the
		 * RTP packets among the UDP packets */
		if(!compress_with_callback(compressor, ip_packet,j))
		{
...
//! [set RTP detection callback]
	...
	printf("***Packet number:%d\tCompressed size: %d\n",packet_count,rohc_packet.len);
	return true;
